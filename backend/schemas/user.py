...
class UserModelSchema(Schema):
    id = fields.Str(
        required=True,
        error_messages={"required": gettext(u"User ID is required"), "invalid": gettext(u"Invalid User ID format")},
    )
    email = get_email_field()
    username = fields.Str(error_messages={"invalid": gettext(u"Invalid username format")})
    provider = EnumField(
        AccountDataProvider,
        by_value=True,
        error_messages={
...
class UserProfilePictureSchema(Schema):
    data = fields.Str(error_messages={"invalid": gettext(u"Invalid profile picture data format")})
